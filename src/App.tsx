...
// Define a type for your list items (optional, but good for TS)
interface LogItem {
    id: number;
    text: string;
}
...
const App = () => {
    ...
    const [logs, setLogs] = useState<LogItem[]>([
        ...
        { id: 2, text: "Connected to Hub." },
        { id: 3, text: "Waiting for input..." },
    ]);
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter' && inputValue.trim()) {
            setLogs([...logs, { id: Date.now(), text: inputValue }]);
            setInputValue("");
        }
    };
    ...
    return (
        ...
        <div className="min-h-screen w-full bg-gray-900 flex flex-col justify-center items-center p-4">
            <div className="w-full max-w-2xl flex flex-col gap-4">
                <h1 className="text-4xl font-bold text-white text-center mb-4 tracking-wider">
                    Distributed Flipper
                </h1>
                ...
                <div className="bg-gray-800 border border-gray-700 rounded-lg p-2 h-64 overflow-y-auto shadow-xl">
                    {logs.length === 0 ? (
                        ...
                        <ul className="space-y-2">
                            {logs.map((log) => (
                                <li key={log.id} className="text-gray-300 font-mono text-sm border-b border-gray-700/50 pb-1 last:border-0">
